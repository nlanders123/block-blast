...
/**
 * Preview which lines would clear if a piece were placed at (startRow, startCol).
 * Returns { rows: number[], cols: number[] } of lines that would complete.
 * Does NOT mutate the board.
 */
function previewLinesToClear(board, piece, startRow, startCol) {
    const boardSize = board.length;
    const shape = piece.shape;

    // Build a set of cells the piece would occupy
    const pieceCells = new Set();
    for (let r = 0; r < shape.length; r++) {
        for (let c = 0; c < shape[r].length; c++) {
            if (shape[r][c] === 1) {
                pieceCells.add(`${startRow + r}-${startCol + c}`);
            }
        }
    }
    ...
    for (let row = 0; row < boardSize; row++) {
        let full = true;
        for (let col = 0; col < boardSize; col++) {
            if (board[row][col] === null && !pieceCells.has(`${row}-${col}`)) {
                full = false;
                break;
            }
        }
